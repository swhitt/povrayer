...
// Typed as an open string map with an optional value rather than left to
// inference: every read here is keyed by an arbitrary dropped extension, so the
// miss has to be visible in the type or imageType() would claim to return a
// keyword for `foo.tiff`.
const IMAGE_TYPE: Readonly<Record<string, string | undefined>> = {
  png: 'png',
  jpg: 'jpeg',
  jpeg: 'jpeg',
  gif: 'gif',
...
/** What a dropped file is allowed to become in the editor. */
export type AssetKind = 'image' | 'include' | 'scene' | 'unknown';
...
export function safeName(filename: string): string {
  // Indexed rather than .pop() because split() always yields at least one
  // segment, so the last element is never actually absent; pop()'s
  // `string | undefined` would buy a `?? ''` branch the gate would then owe a
  // test for a case that cannot happen.
 ...
 */
export function identForFile(filename: string): string {
  const dot = filename.lastIndexOf('.');
 ...
 */
export function uniqueName(name: string, existing: { has: (k: string) => boolean }): string {
 ...
 *
 * @param name the registry filename (as referenced in the FS)
 */
export function assetSnippet(name: string, kind: AssetKind): string {
  if (kind === 'image') {
